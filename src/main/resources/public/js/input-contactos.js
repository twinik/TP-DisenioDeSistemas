...
const getCanalLabel = index => {
    return `<label for="canal-contacto-${index}" class="text-left block mb-2 text-sm font-medium text-gray-900 dark:text-white">Canal de Contacto ${index}</label>`;
};

const getSelectTemplate = index => {
    return `<select id="canal-contacto-${index}" name="canal-${index}" class="mb-5 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"> <option value="Whatsapp">Whatsapp</option><option value="Mail">Mail</option><option value="Telegram">Telegram</option> </select>`;
};
